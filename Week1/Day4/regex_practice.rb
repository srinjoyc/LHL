...
def has_sin?(string)
  ...
  string.match(/\b\d{3}[\s.-]\d{3}[\s.-]\d{3}\b/) ? true:false
...
def grab_all_sins(string)
  sins = string.scan(/\b\d{3}[\s.-]\d{3}[\s.-]\d{3}\b/)
end
...
def hide_all_sins(string)
  ...
  if(!has_sin?(string))
    ...
    sins = string.scan(/\b\d{3}[\s.-]\d{3}[\s.-]\d{3}\b/).join(', ')
    changed_string=sins.gsub(/\b(\d{3})-(\d{3})/, "XXX-XXX")

end

puts "hide_all_sins obfuscates any SINs in the string"
puts hide_all_sins("234-601-142, 350-801-074, 013-601-876-123") == "XXX-XXX-142, XXX-XXX-074, XXX-XXX-876"
...
def format_sins(string)
  ...
  sins = string.scan(/\b\d{3}[\s.-]?\d{3}[\s.-]?\d{3}\b/).join(', ')
  sin_reg = /\b(\d{3})\D?(\d{3})\D?(\d{3})\b/
  changed_string= sins.gsub(sin_reg, '\1-\2-\3')
...
#puts " TESTING ------------------------------format_sins finds and reformat any SINs in the string"
